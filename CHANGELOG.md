...
- `Runner` struct containing server-injected identity fields and client context
- `RunnerRequest` builder for sending client context (instance_id, timestamp, custom fields)
- `get_identity_with_runner()` method on `Client` and `AsyncClient`
- `hostname` field in `Runner` for machine identification
- Export `Runner` and `RunnerRequest` from crate root

### Changed

- **BREAKING**: `Identity` struct now uses `runner` field instead of top-level `pid`, `uid`, `gid`, `process`
- Process identity fields moved into nested `Runner` object
